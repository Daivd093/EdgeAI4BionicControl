...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  HAL_UART_Transmit(&huart2, (uint8_t *)"Ingresa un dígito entre 0 y 6:\r\n", 32, 100);
	  // Receive bloqueante
	  ...
	  if (rx_data[0] < '0' || rx_data[0] > '6') {
	      sprintf(buf, "Carácter no válido: %c\r\n", rx_data[0]);
	  //    HAL_UART_Transmit(&huart2, (uint8_t *)buf, strlen(buf), 100);
	      continue;  // volver al inicio del loop
	  }

	  user_input = (float)(rx_data[0] - '0');
	  ...
	  {
	      printf("Formato completo de entrada: 0x%lX\r\n", (unsigned long)report.inputs->format);

	      // Aquí analizamos solo el tipo base
	      #define AI_BUFFER_FMT_TYPE_MASK 0xFF
	      ai_buffer_format fmt = report.inputs->format;
	      uint8_t base_type = (fmt & AI_BUFFER_FMT_TYPE_MASK);

	      switch (base_type) {
	      	  case 0x40:
				  printf("Formato de entrada detectado: AI_F16\r\n");
				  break;
	      	  case 0x02:
	              printf("Formato de entrada detectado: AI_I8\r\n");
	              break;
	          case 0x10:
	              printf("Formato de entrada detectado: AI_F32\r\n");
	              break;
	          default:
	              printf("Formato de entrada desconocido: 0x%lX\r\n", (unsigned long)base_type);
	              break;
	      }
	 ...
	 // Lee la prediccion
	  y_val = out_data[0];
	  //y_val = half_to_float(out_data[0]);

	  // Imprime la salida de la NN y el tiempo que demoró en hacer inferencia (en microsegundos)
	  //buf_len = sprintf(buf,
